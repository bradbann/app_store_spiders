...
def main():
    schema_dic = {}
    schema_dic['source'] = 'nduoa.com'
    url = 'http://www.nduoa.com/search?q=%E7%AB%A0%E9%B1%BC%E8%BE%93%E5%85%A5%E6%B3%95'
    try:
    ...
    try:
        li_level_str = soup.find('ul', id='searchList')
        version_str = li_level_str.find('span', class_='version').text
        version = re.search(r'[\d\.]+', version_str).group()
        download_count_str = li_level_str.find('span', class_='count').text
        download_count = re.search(r'[\d,]+', download_count_str).group().replace(',','')
    except BaseException:
